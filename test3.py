...
class ImageStitcher(Node):
    ...
    def image_callback(self, msg, camera_index):
        ...
        binary_image = self.adaptive_threshold(cv_image)
        ...
        # Save binary image
        binary_image_filename = os.path.join(self.image_dir, f'camera_{camera_index + 1}_binary_frame_{self.image_counter[camera_index]}.jpg')
        cv2.imwrite(binary_image_filename, binary_image)

        self.image_list[camera_index] = binary_image
    ...
    def custom_stitch_images(self, image_list):
        ...
        homography_matrices = [
            ...
            self.calculate_homography_matrix(CAMERA_POSITIONS[2], CAMERA_POSITIONS[3])
        ]
